...
df = statcast(start_dt='2018-06-01', end_dt='2018-06-02')
...
df.groupby(['pa_id']).size().max()


# In[7]:


padded_all = tf.keras.preprocessing.sequence.pad_sequences(
            [array(df.loc[df['pa_id']=='530255-1', ['release_speed','release_spin_rate']])], 
            maxlen = 10,
            padding='pre',
            value = -100.,
            dtype='float32').reshape(10,2)
...
df['pitch_type_map'] = df['pitch_type_map'].fillna(-1)
...
n_steps_in = df.loc[df['game_date']<=cutoff_date].groupby(['pa_id']).size().max()-2
n_steps_out = 2
...
def model_builder(hp):
    ...
    n_units = hp.Int('units', min_value=36, max_value=72, step=12)
...
tuner.search([encoder_input,decoder_input],decoder_output, 
             epochs=25, validation_split=0.2, callbacks=[stop_early])
...
roc_auc_plot(decoder_otest,preds,0,6)


# In[87]:


roc_auc_plot(decoder_otest,preds,1,6)
